...
if(isset($ubah)){
  //$aksi = ;
  $url = ['pra-rka/ubah-rincian-proses',
        'Tahun' => $Tahun,
        'Kd_Urusan' => $Kd_Urusan,
        'Kd_Bidang' => $Kd_Bidang,
        'Kd_Unit' => $Kd_Unit,
  ...
  $url = ['pra-rka/tambah-rincian-proses'];
  $disable = false;
}
?>
  ...
  <?php $form = ActiveForm::begin(['action' =>$url,'id' => 'tambah_rincian_form']); ?>
    ...
    <?= $form->field($model, 'Kd_Rek_1')->hiddenInput(['value'=> $Kd_Rek_1, 'id'=>'Kd_Rek_1'])->label(false); ?>
      ...
      <?= $form->field($model, 'Nm_Rek_1')->textInput(['maxlength' => true, 'class'=>'form-control input-sm','value'=> $Nm_Rek_1, 'disabled'=>true]) ?>
		...
		<div class="col-md-6">
			<?= $form->field($model, 'Kd_Ap_Pub')->dropDownList($RefApPub, ['class'=>'form-control input-sm']) ?>
			<?= $form->field($model, 'Kd_Sumber')->dropDownList($RefSumberDana, ['class'=>'form-control input-sm']) ?>
		</div>
