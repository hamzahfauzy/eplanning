    ...
    { 
        ...
        */
        ...
        $dataKec = RefKecamatan::find()
                ->where(['Kd_Prov' => $model->Kd_Prov, 'Kd_Kab' => $model->Kd_Kab])
                ->all();
        /*$dataKota = \common\models\RefKabupaten::find()
                ->where(['Kd_Prov' => $model->Kd_Prov, 'Kd_Kab' => $model->Kd_Kab])
                ->one();
        $total = 0;
        //print($dataKota->hasMany(\common\models\search\RefKelurahan::className(), ['Kd_Prov' => 'Kd_Prov', 
        //    'Kd_Kab' => 'Kd_Kab'])->count());exit;
print_r($dataKota->getRefLingkungans()->count());exit;
        /*foreach($dataKota->getRefLingkungans()->all() as $data){
            if ($data->getUsulans()->count() == 0)
                $total++;
        }*/
        //echo ($total.'/'.'2001'.' * 100 = '.($total / 2001 * 100).'%');exit;
