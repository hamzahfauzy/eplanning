...
use emusrenbang\models\KegiatanSkpdSearch;
...
class KegiatanSkpdController extends Controller
{
    ...
    {
        $k=explode('.', $kode);
        $model = RefKegiatan::find()->select('*')->where(['Kd_Urusan'=>$k[0], 'Kd_Bidang'=>$k[1], 'Kd_Prog'=>$k[2], 'Kd_Keg'=>$k[3]])->one();
    ...
    {
        if (($model = KegiatanSkpd::findOne($id)) !== null) {
            return $model;
        } else {
