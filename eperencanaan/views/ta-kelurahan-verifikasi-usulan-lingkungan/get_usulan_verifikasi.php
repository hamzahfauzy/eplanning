<?php
  ...
  foreach ($data as $key => $value) :
    ...
    if ($value["Status_Penerimaan"]==1 || $value["Status_Penerimaan"]==2) {
     // $tombol = '<button class="btn btn-danger btn-tolak" data-kd="'.$Kd_Ta_Forum_Lingkungan.'" data-toggle="modal" data-target="#modal_tolak">Tolak</button> <br/>';
        $tombol = \yii\helpers\Html::Button('Tolak',[
            'class' => 'btn btn-danger btn-tolak btn-sm',
            'data-kd' => $Kd_Ta_Forum_Lingkungan,
            //'id' => 'btn-revisi-asal',
            'data-toggle' => 'modal','tooltip',
            'data-placement' => 'bottom', 
            'title' => 'Jika usulan ditolak',
    ...
    if ($value["Status_Penerimaan"]==3) {
      //$tombol = '<button class="btn btn-success btn-terima" data-kd="'.$Kd_Ta_Forum_Lingkungan.'" data-toggle="modal" data-target="#modal_terima">Terima</button> <br/>';
    
        $tombol = \yii\helpers\Html::Button('Terima',[
            'class' => 'btn btn-success btn-terima btn-sm',
            'data-kd' => $Kd_Ta_Forum_Lingkungan,
            //'id' => 'btn-revisi-asal',
            'data-toggle' => 'modal',
            'data-target' => '#modal_terima',
            'onclick' => '(function(){
            alert("test");
            $.ajax({ 
            type: "GET",
            url:"'.Yii::$app->urlManager->createUrl("ta-kelurahan-verifikasi-usulan-lingkungan/keterangan-langsung").'",
            data:{Kd_Ta_Forum_Lingkungan: '.$Kd_Ta_Forum_Lingkungan.'},
        ...
        $tombol .= \yii\helpers\Html::Button('Diterima dengan Revisi',[
            ...
            $.ajax({ 
            type: "GET",
            url:"'.Yii::$app->urlManager->createUrl("ta-kelurahan-verifikasi-usulan-lingkungan/revisi").'",
            data:{Kd_Ta_Forum_Lingkungan: '.$Kd_Ta_Forum_Lingkungan.'},
            success: function(isi){
            $("#revisi-body").html(isi);
            $("#btn-simpan-revisi").attr("data-kr", '.$Kd_Ta_Forum_Lingkungan.');
            $("#btn-simpan-revisi").show();  
             },
            error: function(){
            alert("failure");
            }
