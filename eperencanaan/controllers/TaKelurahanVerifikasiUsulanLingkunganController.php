...
class TaKelurahanVerifikasiUsulanLingkunganController extends \yii\web\Controller {
    ...
    public function actionGetVerifikasi($Kd_Lingkungan, $status, $bidpem) {
        $posisi = $this->Posisi(); //di ambil dari fungsi Posisi untuk mengambil provinsi sampai kelurahan
		$Kd_Link = ($Kd_Lingkungan=="0") ? ["<>",'Kd_Lingkungan','0'] : ['Kd_Lingkungan'=>$Kd_Lingkungan];
		$bidpem = ($bidpem=="0") ? ["<>",'Kd_Pem','0'] : ['Kd_Pem'=>$bidpem];
	...
	public function actionGetTerima($Kd_Lingkungan, $status, $bidpem) {
        $posisi = $this->Posisi(); //di ambil dari fungsi Posisi untuk mengambil provinsi sampai kelurahan
		$Kd_Link = ($Kd_Lingkungan=="0") ? ["<>",'Kd_Lingkungan','0'] : ['Kd_Lingkungan'=>$Kd_Lingkungan];
		$bidpem = ($bidpem=="0") ? ["<>",'Kd_Pem','0'] : ['Kd_Pem'=>$bidpem];
		...
		*/
        ...
        if ($status == 'revisi') {
            ...
            //old return $this->renderpartial('get_usulan_verifikasi', [
...
//return $this->redirect(['usulan-lingkungan']);
    ...
    public function actionJumlahUsulan() {
        ...
        //$terima = TaKelurahanVerifikasiUsulanLingkungan::find()
        $terima = TaMusrenbangKelurahan::find()
                ->where($Posisi)
                //->andWhere(['in', 'Status_Penerimaan', [1,2]])
                ->count();
   ...
   public function actionLihatriwayat($kode2) {
        $NASRiwayat = TaKelurahanVerifikasiUsulanLingkunganRiwayat::find()
                        ...
                        ->all();
        print_r($NASRiwayat);      
        return $this->renderPartial('modal_lihat_riwayatmodal_lihat_riwayat', [
