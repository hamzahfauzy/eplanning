...
//$No_ID = $model->No_ID;
    ...
    public function actionTambahKegiatanProses()
    {
      ...
      if($model->load($request->post())){
		  ...
		  $pagu_sub_unit = TaPaguSubUnit::find()->where([
			...
			$belanja_rinc_sub = TaBelanjaRincSub::find()->where([
                "Kd_Urusan" => $model->Kd_Urusan,
                "Kd_Bidang" => $model->Kd_Bidang,
                "Kd_Unit" => $model->Kd_Unit,
                "Kd_Sub" => $model->Kd_Sub,
            ...
            $pagu_kegiatan = ($belanja_rinc_sub ? $belanja_rinc_sub : $all_pagu_kegiatan) + $model->Pagu_Anggaran;
            if($pagu_sub_unit->pagu < $pagu_kegiatan)
            {
                echo "Tambah Kegiatan Gagal! Pagu Melebihi Batas yang ditentukan";
                return;
            }
          try {
    ...
    public function actionProgram()
    {
		...
		/*$all_pagu_kegiatan = TaKegiatan::find()->where([
                ...
                "Kd_Bidang" => $unitskpd->Kd_Bidang,
                "Kd_Unit" => $unitskpd->Kd_Unit,
            ])->sum("Pagu_Anggaran");// Diganti menjadi ->*/
			$all_pagu_kegiatan = TaKegiatan::find()->where($PosisiUnit)->sum("Pagu_Anggaran"); //Agar dimunculkan pagu sub unit br RG
