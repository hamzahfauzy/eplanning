...
<div class="misi-form">
  <?php $form = ActiveForm::begin(['action' =>$url,'id' => 'tambah_kegiatan_form']); ?>
  	...
  	<div class="col-md-12">
			...
			<?php //$form->field($model, 'Ket_Kegiatan')->textInput(['maxlength' => true, 'class'=>'form-control input-sm', 'readonly'=>true]) ?>
			<?php //$form->field($model, 'Ket_Kegiatan')->dropDownList($ref_kegiatan, ['prompt'=>'Pilih Sumber', 'class'=>'form-control input-sm selects', 'disabled'=>$disable, 'onchange'=>'alert($(this).val())']) ?>
			<?= $form->field($model, 'Ket_Kegiatan')->dropDownList($ref_kegiatan, ['prompt'=>'Pilih Sumber', 'class'=>'form-control input-sm selects', 'id'=>'pilih-sumber', 'disabled'=>$disable]) ?>
			
			
			<?php //echo '<pre>'; ?>
			<?php //print_r($ref_kegiatan); ?>
			<?php //echo '</pre>'; ?>
		...
		<div class="col-md-6">
			...
			<?= $form->field($model, 'Pagu_Anggaran')->textInput(['maxlength' => true, 'class'=>'form-control input-sm uang', 'id'=>'pagu-anggaran']) ?>
			<?= $form->field($model, 'Pagu_Anggaran_Nt1')->textInput(['maxlength' => true, 'class'=>'form-control input-sm uang','id'=>'pagu-anggarant1']) ?>
				...
				<div class="col-md-12">
					<div class="row box box-primary">
						...
						<div class="col-md-4">
						    ...
						    <input type="text" class="form-control" name="tolak_ukur[<?= $value->Kd_Indikator ?>]" value="<?php if(isset($Tolak_Ukur[$value->Kd_Indikator])) echo $Tolak_Ukur[$value->Kd_Indikator]?>" id="tolak_ukurx<?= $value->Kd_Indikator ?>">
						  </div>
						</div>
						<div class="col-md-3">
							<div class="form-group">
						    <label >Target</label>
						    <input type="text" class="form-control angka" name="target[<?= $value->Kd_Indikator ?>]" value="<?php if(isset($Target_Angka[$value->Kd_Indikator])) echo $Target_Angka[$value->Kd_Indikator] ?>"  id="targetx<?= $value->Kd_Indikator ?>">
						  </div>
						</div>
						<div class="col-md-3">
							<div class="form-group">
							...
							<select  class="form-control input-sm selects__" name="target_satuan[<?= $value->Kd_Indikator ?>]" id="target_satuanx<?= $value->Kd_Indikator ?>">
...
// pilih sumber
$('#pilih-sumber').on('change', function () {
	
	var data = $(this).val();
	var arr = data.split('|');
	
	$('#tolak_ukurx3').val (arr[2]);
	$('#tolak_ukurx7').val (arr[2]);

	
	
	
	$('#pagu-anggaran').val(arr[6]);
	$('#pagu-anggarant1').val(arr[7]);
	$('#targetx3').val (arr[4]);
	$('#targetx7').val (arr[5]);
	$('#target_satuanx3').val (arr[3]);
	$('#target_satuanx7').val(arr[3]);
	console.log(arr);
