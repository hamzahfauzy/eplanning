...
if(isset($ubah)){
	//$aksi = ;
	$url = ['pra-rka-apbn/ubah-kegiatan-proses',
				'Tahun' => $Tahun,
				'Kd_Urusan' => $Kd_Urusan,
				'Kd_Bidang' => $Kd_Bidang,
				'Kd_Unit' => $Kd_Unit,
	...
	$url = ['pra-rka-apbn/tambah-kegiatan-proses'];
	$disable = false;
}
?>
<div class="misi-form">
  <?php $form = ActiveForm::begin(['action' =>$url,'id' => 'tambah_kegiatan_form']); ?>
  	...
  	<div class="col-md-12">
		...
		<div class="col-md-6">
			...
			<?= $form->field($model, 'Kd_Sumber')->dropDownList(['1'=>'APBN', '2'=>'Bantuan Keuangan Provinsi'], ['prompt'=>'Pilih Sumber', 'class'=>'form-control input-sm selects']) ?>
		...
		?>
		<!-- <hr> -->
		<!-- <h3 align="center">Pilih OPD Provinsi</h3>
		<div class="col-md-3">
			<?php //echo $form->field($model, 'Kd_Urusan_Prov')->dropDownList($RefUrusanProv, ['prompt'=>'Pilih Status', 'class'=>'form-control input-sm', 'id'=>'Kd_Urusan_Prov']) ?>
		</div>
		<div class="col-md-3">
			<?php //echo  $form->field($model, 'Kd_Bidang_Prov')->dropDownList($RefBidangProv, ['prompt'=>'Pilih', 'class'=>'form-control input-sm', 'id'=>'Kd_Bidang_Prov']) ?>
		</div>
		<div class="col-md-3">
			<?php //echo  $form->field($model, 'Kd_Unit_Prov')->dropDownList($RefUnitProv, ['prompt'=>'Pilih', 'class'=>'form-control input-sm', 'id'=>'Kd_Unit_Prov']) ?>
		</div>
		<div class="col-md-3">
			<?php //echo  $form->field($model, 'Kd_Sub_Prov')->dropDownList($RefSubUnitProv, ['prompt'=>'Pilih', 'class'=>'form-control input-sm', 'id'=>'Kd_Sub_Prov']) ?>
		</div>
		<div class="clearfix"></div> -->
...
$("#tambahKamusSave").click(function(){
  $('#tambahKamusSave').attr('disabled', true);
	$.ajax({ 
    type: "POST",
    url:'index.php?r=pra-rka-prov/tambah-kamus-proses',
...
//========prov=========//
$('#Kd_Urusan_Prov').change(function(){
  Kd_Urusan=$(this).val();
  //alert('index.php?r=ajax/get-bidang-prov&Kd_Urusan='+Kd_Urusan);
  $.post('index.php?r=ajax/get-bidang-prov&Kd_Urusan='+Kd_Urusan, function(data, status){
      $('#Kd_Bidang_Prov').html(data);
  })
});

$('#Kd_Bidang_Prov').change(function(){
  Kd_Urusan=$("#Kd_Urusan_Prov").val();
  Kd_Bidang=$(this).val();

  $.post('index.php?r=ajax/get-unit-prov&Kd_Urusan='+Kd_Urusan+'&Kd_Bidang='+Kd_Bidang, function(data, status){
      $('#Kd_Unit_Prov').html(data);
  })
});

$('#Kd_Unit_Prov').change(function(){
  Kd_Urusan=$("#Kd_Urusan_Prov").val();
  Kd_Bidang=$("#Kd_Bidang_Prov").val();
  Kd_Unit=$(this).val();
  //alert('index.php?r=ajax/get-sub-unit-prov&Kd_Urusan='+Kd_Urusan+'&Kd_Bidang='+Kd_Bidang+'&Kd_Unit='+Kd_Unit);
  $.post('index.php?r=ajax/get-sub-unit-prov&Kd_Urusan='+Kd_Urusan+'&Kd_Bidang='+Kd_Bidang+'&Kd_Unit='+Kd_Unit, function(data, status){
      $('#Kd_Sub_Prov').html(data);
  })
});
